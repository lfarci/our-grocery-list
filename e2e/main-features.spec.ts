...
test.describe('Grocery List - Main Features', () => {
  ...
  test('should display empty list state on initial load', async ({ page }) => {
  ...
  test('should maintain list across add, toggle, and delete operations', async ({ page }) => {
    ...
    // Mark Item B as done
    await page.getByRole('checkbox', { name: 'Mark Item B as done' }).check();
    ...
    await expect(page.getByText('Item B')).toBeVisible();
    await expect(page.getByText('Item C')).toBeVisible();
    await expect(page.getByText('With notes')).toBeVisible();
    
    // Verify Item B is still marked as done
    await expect(page.getByRole('checkbox', { name: 'Mark Item B as done' })).toBeChecked();
  });
  ...
  test('should clear form validation error when valid input is provided', async ({ page }) => {
    ...
    await expect(page.getByText('Valid Item')).toBeVisible();
  });
  ...
  test('should handle items with special characters', async ({ page }) => {
    ...
    await expect(page.getByText('Coffee & Tea (organic)')).toBeVisible();
    await expect(page.getByText('50% off - $5.99')).toBeVisible();
  });
  ...
  test('should maintain item order when marking items as done', async ({ page }) => {
    ...
    await page.getByRole('checkbox', { name: 'Mark Second as done' }).check();
    await page.getByRole('checkbox', { name: 'Mark Third as done' }).check();
  ...
  test('should display checkbox controls as touch-friendly', async ({ page }) => {
    ...
    const checkbox = page.getByRole('checkbox', { name: 'Mark Test Item as done' });
